...
AUTO_REFRESH_ALLOWED_TABS = {"Synthèse", "Marchés", "Sélection d'Actifs", "Simulation & Opérations"}
...
DEFAULT_SNAPSHOT_MIN_SECONDS = 5
DEFAULT_SNAPSHOT_MIN_DELTA = 0.01
...
def simulate_order_execution(
    ...
    # spread_bps représente le spread bid-ask total; on applique le demi-spread à l'exécution
    spread_factor = spread_bps / 20_000.0  # demi-spread
    slip_factor = slippage_bps / 10_000.0  # slippage complet
    if side == "BUY":
...
def init_db(conn: sqlite3.Connection) -> None:
    ...
    conn.execute(
        "INSERT OR IGNORE INTO settings(key, value) VALUES ('auto_refresh_enabled', '1')"
    )
    conn.execute(
    ...
    # ──────────────────────────────────────────────────────────────
    # Migration : forcer les settings critiques pour les DB existantes
    # qui ont été créées avec d'anciens defaults (auto_refresh=0, snapshot_min_delta=1.0)
    # → Sans auto-refresh, aucun snapshot n'est créé automatiquement
    # → Avec un snapshot_min_delta élevé, les petites variations ne sont pas capturées
    # → La courbe de pricing apparaît plate.
    # ──────────────────────────────────────────────────────────────
    _critical_migrations = {
        "auto_refresh_enabled": "1",
        "snapshot_min_seconds": str(DEFAULT_SNAPSHOT_MIN_SECONDS),
        "snapshot_min_delta": str(DEFAULT_SNAPSHOT_MIN_DELTA),
        "refresh_seconds": str(DEFAULT_REFRESH_SECONDS),
    }
    for key, target_value in _critical_migrations.items():
        current = conn.execute("SELECT value FROM settings WHERE key = ?", (key,)).fetchone()
        if current is not None:
            cur_val = str(current["value"]).strip()
            # Migrer si l'ancienne valeur est moins performante
            if key == "auto_refresh_enabled" and cur_val == "0":
                conn.execute("UPDATE settings SET value = ? WHERE key = ?", (target_value, key))
            elif key == "snapshot_min_delta" and float(cur_val or "1") > 0.5:
                conn.execute("UPDATE settings SET value = ? WHERE key = ?", (target_value, key))
            elif key == "snapshot_min_seconds" and int(float(cur_val or "10")) > 10:
                conn.execute("UPDATE settings SET value = ? WHERE key = ?", (target_value, key))
            elif key == "refresh_seconds" and int(float(cur_val or "10")) > 10:
                conn.execute("UPDATE settings SET value = ? WHERE key = ?", (target_value, key))
...
def upsert_snapshot(
    ...
    if last is not None:
        ...
        if explicit_event is None:
            # Toujours respecter l'intervalle minimum entre snapshots
            if elapsed < max(min_seconds, 1):
                return
            # Enregistrer un snapshot dès qu'il y a des positions investies,
            # même si la variation est minime — c'est ce qui fait vivre la courbe.
...
def compute_positions(transactions: pd.DataFrame, accounting_method: str = DEFAULT_ACCOUNTING_METHOD) -> pd.DataFrame:
    ...
    for tx in transactions.itertuples(index=False):
        ...
        ledger = ledgers.setdefault(
            symbol,
            {
                ...
                "currency": currency,
                "fx_to_base_weighted": [],  # list of (qty, fx) tuples for weighted avg
            },
        )
        ledger["currency"] = currency
        if fx_to_base > 0:
            ledger["fx_to_base_weighted"].append((qty, fx_to_base))
    ...
    for ledger in ledgers.values():
        ...
        fx_pairs = ledger["fx_to_base_weighted"]
        total_qty_fx = sum(q for q, _ in fx_pairs)
        avg_fx = sum(q * f for q, f in fx_pairs) / total_qty_fx if total_qty_fx > 0 else 1.0
        rows.append(
            {
...
def infer_currency(symbol: str, quote_currency: str | None, base_currency: str) -> str:
    ...
    if s.endswith(".TW"):
        return "TWD"
    # Symboles sans suffixe → probablement cotés en USD (US exchanges)
    return "USD"
...
def compute_portfolio_state(
    ...
    for pos in positions.itertuples(index=False):
        ...
        last = float(q.get("last", np.nan))
        if np.isnan(last) or last <= 0:
            # Pas de cotation disponible — on garde le prix de revient pour l'affichage
            # mais on marque la position comme non valorisée en temps réel.
    ...
    *,
    ...
    with st.form(f"{key_prefix}_form_quick_sell"):
        ...
        if submitted:
            ...
            # Re-fetch prix frais avant exécution de vente rapide
            fetch_realtime_quotes.clear()
            fetch_polygon_snapshot_quotes.clear()
            fresh_sell_quote = fetch_realtime_quotes((str(symbol),))
            if not fresh_sell_quote.empty:
                fresh_sell_price = safe_float(fresh_sell_quote.iloc[0].get("last", np.nan) if hasattr(fresh_sell_quote.iloc[0], "get") else getattr(fresh_sell_quote.iloc[0], "last", np.nan), np.nan)
                if not np.isnan(fresh_sell_price) and fresh_sell_price > 0:
                    unit_quote = float(fresh_sell_price)
...
def main() -> None:
    ...
    if "auto_refresh_enabled" not in st.session_state:
        st.session_state["auto_refresh_enabled"] = get_setting(conn, "auto_refresh_enabled", "1") == "1"
    ...
    with tabs[1]:
        ...
        with st.container():
            ...
            if submitted:
                # CORRIGÉ : rafraîchir le prix AVANT exécution pour éviter un prix caché stale
                fetch_realtime_quotes.clear()
                fetch_quotes.clear()
                fresh_quotes = fetch_realtime_quotes((symbol,))
                if not fresh_quotes.empty:
                    fresh_row = fresh_quotes[fresh_quotes["symbol"] == symbol]
                    if not fresh_row.empty:
                        fresh_price = safe_float(fresh_row.iloc[0].get("last", np.nan), np.nan)
                        if not np.isnan(fresh_price) and fresh_price > 0:
                            price = float(fresh_price)
                            market_quote = fresh_row.iloc[0].to_dict()
                            market_currency = infer_currency(symbol, str(market_quote.get("currency", "")), state["base_currency"])
                ...
                if quantity <= 0:
                    ...
                    # Re-fetch prix frais avant exécution pour éviter un prix stale du cache
                    fetch_realtime_quotes.clear()
                    fetch_polygon_snapshot_quotes.clear()
                    fresh_quote = fetch_realtime_quotes((symbol,))
                    if not fresh_quote.empty:
                        fresh_price = safe_float(fresh_quote.iloc[0].get("last", np.nan) if hasattr(fresh_quote.iloc[0], "get") else getattr(fresh_quote.iloc[0], "last", np.nan), np.nan)
                        if not np.isnan(fresh_price) and fresh_price > 0:
                            price = float(fresh_price)
                            market_quote = fresh_quote.iloc[0].to_dict() if hasattr(fresh_quote.iloc[0], "to_dict") else market_quote
                    execution = simulate_order_execution(
                        side=side,
                        order_type=order_type,
